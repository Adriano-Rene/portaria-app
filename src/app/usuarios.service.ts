...
export class UsuariosService {
  ...
  atualizar(usuario :Usuario) : Observable<any> {
    return this.http.put<Usuario>(`http://localhost:8080/usuario/${usuario.id}`, usuario)
  }

  deletar(usuario:Usuario): Observable<any>{
    return this.http.delete<any>(`http://localhost:8080/usuario/${usuario.id}`)
  }
...
getUsuarioById(id:Number): Observable<Usuario>{
      return this.http.get<any>(`http://localhost:8080/usuario/${id}`)
     }
}
